...
namespace KcpProject
{
    ...
    public class KCP
    {
        ...
        {
            conv = conv_;
            Console.WriteLine($"[KCP] Initialized with Conv: {conv}");
        ...
        public int PeekSize()
        {
            if (rcv_queue == null) return -1;
            if (0 == rcv_queue.Count) return -1;

            var seq = rcv_queue[0];
            if (seq == null) return -1;
            if (seq.data == null) {
                Console.WriteLine($"[KCP] PeekSize Error: seq.data is null! seq={seq.GetHashCode()}");
                return -1;
            }
        ...
        public int Recv(byte[] buffer, int index, int length)
        {
            ...
            // First pass: copy data and count
            foreach (var seg in rcv_queue)
            {
                // copy fragment data into buffer.
                int copyLen = seg.data.ReadableBytes;
                if (n + copyLen > buffer.Length) {
                    // This should be caught by peekSize check, but just in case
                    break; 
                }
                
                Buffer.BlockCopy(seg.data.RawBuffer, seg.data.ReaderIndex, buffer, n, copyLen);
                n += copyLen;
            ...
            if (count > 0)
            {
                // Collect segments to recycle
                var segmentsToRecycle = new List<Segment>();
                for (int i = 0; i < count; i++) {
                    segmentsToRecycle.Add(rcv_queue[i]);
                }
                ...
                // Recycle
                foreach (var seg in segmentsToRecycle) {
                    // Console.WriteLine($"[KCP] Recv: Recycling seq={seg.GetHashCode()}");
                    Segment.Put(seg);
                }
            }
        ...
        public int Input(byte[] data, int index, int size, bool regular, bool ackNoDelay)
        {
            ...
            while (true)
            {
                ...
                if (conv != conv_) {
                    Console.WriteLine($"[KCP] Conv mismatch: Recv={conv_} Expected={conv}");
                    return -1;
                }
